...
def register(mcp: FastMCP) -> None:
    ...
    @mcp.tool(
        ...
        description="Generate a Prashna (horary) chart for a question asked at this moment.\nPlanetary positions at the time and place the question is asked are\nused to answer the question — no birth data required.\nSource: Prashna Marga and classical horary texts.\n\nCRITICAL INPUT CONSTRAINT: The question parameter accepts ONLY a single\ncategory keyword — not a full sentence. Valid values: self, wealth,\nsiblings, property, children, health, marriage, death, travel, career,\ngains, loss. Passing a full sentence ('Will I get a job?') returns 422.\nPass 'career' not 'Will I get a job this year?'\n\nOUTPUT CONTRACT (response_format=json):\ndata.ayanamsa, data.question (the keyword submitted),\ndata.primary_house (int, house mapped from the keyword),\ndata.ithsala_applying (bool), data.ithsala_separating (bool),\ndata.query_utc (ISO UTC of computation)\ndata.lagna — { rashi_index, rashi, longitude, lord }\ndata.house_analysis — { house, rashi_index, rashi, lord,\n  lord_dignity, lord_house, lord_longitude, occupants[] }\ndata.moon — { longitude, rashi_index, rashi, nakshatra, phase\n  ('waxing' or 'waning'), dignity, void_of_course (bool —\n  Moon VOC is a classical negative indicator: 'nothing will\n  come of the matter'), afflicted_moon (bool),\n  applying_to_benefic (bool) }\ndata.verdict — { verdict (string: 'favorable', 'mixed', or\n  'unfavorable'), confidence ('high', 'medium', or 'low'),\n  score (int — negative = unfavorable, positive = favorable) }\ndata.planets{} — keyed by planet: longitude, rashi_index, rashi,\n  house (int, from Prashna Lagna), is_retrograde, dignity\ndata.house_cusps{} — keyed by '1'–'12':\n  rashi_index, rashi, lord, longitude\n\nERROR CONTRACT: Full-sentence question → 422. Invalid category\nkeyword → 422. Invalid date/time format → 422.\n\nDo not use for ongoing life analysis — use asterwise_get_natal_chart\nfor that.",
        annotations=STANDARD_ANNOTATIONS,
    )
